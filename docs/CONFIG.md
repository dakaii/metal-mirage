...
| `primary:adminCidr` | no | `0.0.0.0/0` | Source for Talos APIs + etcd. azure-metal-sim `up.sh` defaults to **`0.0.0.0/0`** (CGNAT jumps `/24`s); lock with `ADMIN_CIDR=…` or `ADMIN_CIDR_PREFIX_LEN=24\|32` |
