...
| `primary:adminCidr` | no | `0.0.0.0/0` | Source for Talos APIs + etcd. azure-metal-sim `up.sh` auto-sets a **/24** (CGNAT); use `ADMIN_CIDR_PREFIX_LEN=32` or `ADMIN_CIDR=…` for a host lock; intentional `0.0.0.0/0` is left alone |
