...
- DR drill steps: [docs/DR.md](docs/DR.md) (TM `:80/healthz` vs witness `:6443/readyz`; witness is advisory only). Operator cutover helper: `./scripts/failover-promote.sh` (suspend Flux + scale standby; optional `--disable-primary-tm`; `--failback`). Opt-in webhook/GHA promote: [docs/AUTO-FAILOVER.md](docs/AUTO-FAILOVER.md).
